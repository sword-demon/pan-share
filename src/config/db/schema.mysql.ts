...
export const panShare = table(
  'pan_share',
  {
    ...
    diskType: varchar('disk_type', { length: 50 }).notNull(), // 网盘类型: baidu, aliyun, quark, xunlei, 115, other
    shareUrl: text('share_url').notNull(), // 分享链接
    shareCode: varchar('share_code', { length: 50 }), // 提取码
    expiredAt: timestamp('expired_at'), // 过期时间
    status: varchar('status', { length: 50 }).notNull().default('pending'), // pending, published, rejected
